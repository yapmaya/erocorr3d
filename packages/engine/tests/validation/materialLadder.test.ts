...
//
// selectPipingMaterial() §10.3.2 karar merdiveninin, hem kaynak dokümanın
// (kimliği anonim tutulan iç proje dokümanı) kendi gerçek akış örnekleriyle hem de
...
import { MATERIAL_LADDER_CASES } from "../../src/fixtures/referenceFacilityValidationData";
...
describe("selectPipingMaterial — Tablo 10-3'ün gerçek akış örnekleri", () => {
  for (const testCase of MATERIAL_LADDER_CASES) {
    it(`Hat ${testCase.streamId}: SLC=${testCase.slcMm}mm → CS + ${testCase.expectedCaMm}mm CA`, () => {
...
describe("selectPipingMaterial — servis içi muayene istisnası (3-6mm bandı)", () => {
  it("inServiceInspectionPossible=true iken 3-6mm bandında 3.0mm CA yeterli görülür", () => {
    const result = selectPipingMaterial({
      requiredCorrosionAllowanceMm: 3.45, // Hat L3 ile aynı SLC
